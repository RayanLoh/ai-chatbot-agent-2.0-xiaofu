  ...
  try {
    ...
    const { pipe, abort } = render(url, {
      onShellError(err) {
        ...
        // In development, return the stack trace to the browser to aid debugging.
        if (!isProduction && err && err.stack) {
          res.send(`<pre style="white-space:pre-wrap;">${err.stack}</pre>`)
        } else {
      ...
      onShellReady() {
       ...
       res.set({ 
          ...
          'Content-Security-Policy': [
            "default-src 'self'",
            // 允许 Vite 的 eval 和 Google OAuth 脚本
            "script-src 'self' 'unsafe-inline' 'unsafe-eval' https://accounts.google.com https://www.gstatic.com",
            // 关键：允许连接到你的 ngrok 地址和本地热更新热点
            "connect-src 'self' https://hearselike-jake-overconscientiously.ngrok-free.dev ws://localhost:5173",
